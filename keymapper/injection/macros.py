    ...
    def exitpgm(self, var1, var2=None):
        ...
        async def exitpgm(handler):
            ...
            try:
                ...
                result = subprocess.check_output([exitpgm, exit_var1, exit_var2]).decode().splitlines()[0]
                logger.debug('Exit program returned "%s"', result)
                if result != '':
                    exit_macro=parse(result,self.mapping)
                    if isinstance(exit_macro, _Macro):
                        await exit_macro.run(handler)
                    else:
                        logger.error('Exit program returned invalid macro: "%s"', result)
